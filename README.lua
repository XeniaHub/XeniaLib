...
local CoreGui = game:GetService("CoreGui"):WaitForChild("RobloxGui"):WaitForChild("Modules")
...
do
	...
	function Funcs:AddDropdown(Idx, Info)
		...
		function Dropdown:SetValues()
			...
			local Y = math.clamp(Count * 20, 0, MAX_DROPDOWN_ITEMS * 20)
...
function Library.CreateWindow(info)
	...
	local titletext = info and info.Title and info.Title.Name or 'hub'
	local sizetiX = Library:GetTextBounds(titletext, Enum.Font.Code, 16)
	local TitleOuter = Library:Create('Frame', {
		...
		Size = UDim2.new(0, sizetiX+40, 0, 28),
		ZIndex = 1,
		Parent = Inner
	})
	...
	if info and info.Title and info.Title.Category then
		local CategoryOuter = Library:Create('Frame', {
			BorderColor3 = Color3.new(0, 0, 0),
			Position = UDim2.new(0, (44+(sizetiX+40))+4, 0, 10),
			Size = UDim2.new(0, (495-(sizetiX+40))-4, 0, 28),
			ZIndex = 1,
			Parent = Inner
		})
	...
	function Window:AddTab(Name,Info)
		...
		local ScrollingLeftSide = Library:Create('ScrollingFrame', {
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
		...
		local ScrollingRightSide = Library:Create('ScrollingFrame', {
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
		...
		local function chargesizescrolling(frame)
			local nun = 0
			local SizeY = 0
		...
		ScrollingLeftSide.CanvasSize = UDim2.new(0, 0, 0, chargesizescrolling(RightSide))
		ScrollingRightSide.CanvasSize = UDim2.new(0, 0, 0, chargesizescrolling(LeftSide))
		
		LeftSide.ChildAdded:Connect(function(obj)
			ScrollingLeftSide.CanvasSize = UDim2.new(0, 0, 0, chargesizescrolling(LeftSide))
		end)
		RightSide.ChildAdded:Connect(function(obj)
			ScrollingRightSide.CanvasSize = UDim2.new(0, 0, 0, chargesizescrolling(RightSide))
		end)
	...
	InputService.InputBegan:Connect(function(Input, Processed)
		if Input.KeyCode == Enum.KeyCode[Options.MenuKeybind and Options.MenuKeybind.Value or "RightControl"] and not Processed and not opencd then
			if Outer.Visible then
				TweenService:Create(Outer,TweenInfo.new(.25,Enum.EasingStyle.Linear,Enum.EasingDirection.Out),{Size = UDim2.fromOffset(0,0)}):Play()
				delay(.25,function()
					Outer.Visible = false
				end)
			else
				Outer.Visible = true
				TweenService:Create(Outer,TweenInfo.new(.25,Enum.EasingStyle.Linear,Enum.EasingDirection.Out),{Size = UDim2.fromOffset(550,650)}):Play()
			end
			opencd = true
			wait(.25)
			opencd = false
